...
const float RING_THICKNESS = 0.03;             // Ring width in normalized coords
const float CURSOR_WIDTH_CHANGE_THRESHOLD = 0.5; // Triggers ripple if cursor width changes by this fraction
vec4 COLOR = vec4(0.35, 0.36, 0.44, 0.2); // change to iCurrentCursorColor for your cursor's color
const float BLUR = 10.0;                    // Blur level in pixels
